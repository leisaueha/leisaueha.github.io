...
date: 2026-07-19
description: What is Langevin sampling, and how can we use it to sample energy-based distributions?
tags:
    - langevin sampling
    - probability
---
...
Code that generates this visualization (generated by ChatGPT): [langevin_2d_video.py](https://github.com/leisaueha/stuff/blob/main/langevin_2d_video.py)
...
Assume the molecules hit at a rate $\lambda$. Then the total number of collisions during the interval $\Delta t$ is
...
With a fixed $s'\in[t,t+\Delta t]$, the inner integral is $1$. Hence,
...
(I honestly think this logic is overreaching, as the form of $v$ is not $v=v(0)e^{-t/\tau}$.)
...
This is the Euler–Maruyama approximation. Over one sufficiently small time step, we approximate the score by its value at the beginning of the step, $\nabla\ln p(x_k)$. This works when the score changes smoothly; otherwise, we need to choose a smaller $\epsilon$. Hence,
